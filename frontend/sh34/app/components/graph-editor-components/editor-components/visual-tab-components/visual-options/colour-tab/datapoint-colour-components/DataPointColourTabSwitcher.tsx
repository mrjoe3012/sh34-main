...
import { DataPointScaleOption } from './DataPointScaleOption';

interface dataPointColourTabProps {
    switchTabFunction: (someComponent: JSX.Element) => void;
}

export const DataPointColourTabSwitcher = (props : dataPointColourTabProps) => {
    ...
    const handleConstantClicked = () => {
        ...
        props.switchTabFunction(<DataPointConstantOption />)
    }

    const handleScaleClicked = () => {
        setConstantClicked(false);
        setScaleClicked(true);
        props.switchTabFunction(<DataPointScaleOption />)
    }
